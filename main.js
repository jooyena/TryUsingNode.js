...
var app = http.createServer(function(request,response){
    var _url = request.url;//얘가 query string?
	var queryData = url.parse(_url,true).query;//parse는 조사라는 뜻
	console.log(queryData.id);
    
	if(_url == '/'){
      _url = '/index.html';//정적인 파일만 가져오고 있다
    }
    if(_url == '/favicon.ico'){
    ...
    response.end(fs.readFileSync(queryData.id));// 사용자가 선택한 url에 따라 반응을 하는 코드
	//nodejs가 그 경로에 해당되는 파일을 읽어와서 가져온다.
	
	//http://opentutorials.org:3000/main?id=HTML&page=12
	// 프로토콜 // 호스트 : 포트 / 경로 ? 쿼리 스트링(query string)<<이거에 따라 다른 페이지
...
var app = http.createServer(function(request,response){
    ...
    console.log(queryData.id);
    if(_url == '/'){
      //_url = '/index.html';//홈으로 갔을때 저기 있는 동적 html이 실행 되게 해라 But 동적일 떄는 필요 없어
	  title = 'welcome'
    }
...
<body>
  ...
  title="html5 speicification">Hypertext Markup Language (HTML)
  </a> is the standard markup language for <strong>creating 
  <u>web</u> pages</strong> and web applications.
  Web browsers receive HTML documents from a web server or from local storage 
  and render them into multimedia web pages. HTML describes 
  ...
  such as interactive forms, may be embedded into the rendered page. It provides
